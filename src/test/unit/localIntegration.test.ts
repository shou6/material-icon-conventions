 ...
 * このワークスペースを VS Code で開いていると、.vscode-test/extensions への MIT のインストールが
 * フォルダの rename で EPERM になる（ワークスペースの外なら成功する）。
...
suite('installDependencyArgs', () => {
  ...
  test('実際の package.json では MIT を入れる', () => {
    const manifest = JSON.parse(fs.readFileSync(path.join(ROOT, 'package.json'), 'utf8')) as {
      extensionDependencies?: string[];
    };
    const args = installDependencyArgs(manifest, '/tmp/ext', '/tmp/cli');
    assert.ok(args.includes('PKief.material-icon-theme'), args.join(' '));
  });
});
